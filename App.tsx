...
import { Menu, Search, User as UserIcon, HelpCircle, X, ChevronRight, Pin as PinIcon, BookOpen } from 'lucide-react';
...
// Enhanced placeholder data for homepage with higher quality aesthetic
...
function App() {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem('pantry_chef_user');
    const storedHistory = localStorage.getItem('pantry_chef_history');
    const storedPins = localStorage.getItem('pantry_chef_pins');
    
    if (storedUser) setUser(JSON.parse(storedUser));
    if (storedHistory) setHistory(JSON.parse(storedHistory));
    if (storedPins) setPinnedRecipes(JSON.parse(storedPins));
  }, []);

  useEffect(() => {
    if (user) localStorage.setItem('pantry_chef_user', JSON.stringify(user));
  }, [user]);

  useEffect(() => {
    localStorage.setItem('pantry_chef_history', JSON.stringify(history));
  }, [history]);

  useEffect(() => {
    localStorage.setItem('pantry_chef_pins', JSON.stringify(pinnedRecipes));
  }, [pinnedRecipes]);
  ...
  const handleSearch = async (input: { text?: string; imageBase64?: string; mimeType?: string; audioBase64?: string }) => {
    ...
    try {
      ...
      const newHistoryItem: HistoryItem = {
        ...
        timestamp: Date.now(),
        queryType: input.imageBase64 ? 'image' : input.audioBase64 ? 'audio' : 'text',
        queryPreview: input.text || (input.imageBase64 ? 'Fridge Scan' : 'Voice Query'),
  ...
  const togglePin = (recipe: Recipe) => {
    if (pinnedRecipes.find(r => r.id === recipe.id)) {
  ...
  return (
    <div className={`min-h-screen flex flex-col relative overflow-x-hidden ${theme === 'dark' ? 'bg-slate-900 text-white' : 'bg-gray-50'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
             <button 
               ...
               className="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-bold text-gray-500 hover:text-chef-orange hover:bg-orange-50 dark:hover:bg-orange-950/20 rounded-xl transition-all"
             >
                <HelpCircle size={20} />
             ...
             <button 
               ...
               className="p-3 text-gray-500 hover:bg-gray-100 rounded-2xl dark:hover:bg-slate-800 transition-all"
               title="Settings"
             >
                <UserIcon size={22} />
             </button>
             ...
             {user ? (
               <div className="flex items-center gap-3 pl-4 border-l border-gray-100 dark:border-slate-800 ml-2">
                 ...
                 <div className="hidden sm:block">
                   <p className="text-xs font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Chef</p>
               ...
               <button 
                 ...
                 className="bg-chef-dark dark:bg-chef-orange text-white px-6 py-2.5 rounded-2xl text-sm font-bold hover:shadow-xl transition-all active:scale-95 ml-2"
               >
                 Sign In
               </button>
             )}
      ...
      <div className="flex-1 flex relative z-10">
        ...
        {/* Sidebar History */}
        <aside className={`
            fixed lg:sticky top-20 left-0 h-[calc(100vh-80px)] w-80 bg-white/80 backdrop-blur-2xl border-r border-gray-100 dark:bg-slate-900/80 dark:border-slate-800
        ...
        `}>
          <div className="p-8 h-full flex flex-col">
            <h3 className="font-black text-gray-400 uppercase text-[10px] tracking-[0.2em] mb-6 flex items-center gap-2">
               <Search size={14} className="text-chef-orange" /> Recent History
            </h3>
            <div className="flex-1 overflow-y-auto scrollbar-hide space-y-4">
              {history.length === 0 ? (
                <div className="text-center py-10">
                  <div className="w-12 h-12 bg-gray-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center mx-auto mb-3">
                    <BookOpen size={20} className="text-gray-300" />
                  </div>
                  <p className="text-xs text-gray-400 font-medium px-6">Your search history will appear here.</p>
                </div>
              ) : (
                history.map(item => (
                  <div 
                    ...
                    className="p-4 rounded-2xl bg-white dark:bg-slate-800 hover:bg-orange-50 dark:hover:bg-orange-950/20 cursor-pointer border border-gray-50 dark:border-slate-700 hover:border-orange-100 transition-all group shadow-sm"
                  >
                    <div className="flex justify-between items-start mb-2">
                       <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{new Date(item.timestamp).toLocaleDateString()}</span>
                       <ChevronRight size={14} className="text-gray-300 group-hover:text-chef-orange transition-transform group-hover:translate-x-1" />
                    </div>
                    <p className="font-bold text-sm text-gray-800 truncate dark:text-gray-200">
            ...
            {pinnedRecipes.length > 0 && (
                <div className="mt-8 pt-8 border-t border-gray-100 dark:border-slate-800">
                  <h3 className="font-black text-gray-400 uppercase text-[10px] tracking-[0.2em] mb-6 flex items-center gap-2">
                    <PinIcon size={14} className="text-chef-yellow" /> Pinned Recipes
                  </h3>
                  <div className="space-y-3">
                     {pinnedRecipes.slice(0, 5).map(recipe => (
                        <div key={recipe.id} className="group flex items-center justify-between gap-3 text-sm font-bold text-gray-700 dark:text-gray-300 p-3 bg-gray-50 dark:bg-slate-800/50 rounded-2xl hover:bg-chef-yellow/10 transition-colors cursor-pointer">
                           <div className="flex items-center gap-3 truncate">
                             <div className="w-2 h-2 rounded-full bg-chef-yellow shrink-0"></div>
        ...
        <main className="flex-1 p-6 lg:p-12 max-w-7xl mx-auto w-full">
           ...
           <div className={`text-center transition-all duration-700 ${recipes.length > 0 ? 'mb-12' : 'my-24'}`}>
              <h1 className="text-5xl md:text-7xl font-black mb-8 text-gray-900 dark:text-white tracking-tighter leading-[0.9]">
                Turn <span className="text-chef-orange italic">Leftovers</span> into <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-chef-yellow via-chef-orange to-red-500">Gourmet Meals</span>
              </h1>
              <p className="text-lg md:text-xl text-gray-500 mb-12 max-w-2xl mx-auto dark:text-gray-400 font-medium leading-relaxed">
                Our AI-powered kitchen analyzes your fridge contents to suggest instant, professional-grade recipes that eliminate food waste.
              </p>
               ...
               <div className="mt-20">
                 <div className="flex items-center justify-between mb-8">
                   <h2 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-3">
                     <span className="bg-orange-100 dark:bg-orange-900/20 p-2 rounded-2xl">🔥</span> Trending Suggestions
                   </h2>
                   <button onClick={() => openSettings('help')} className="text-sm font-bold text-chef-orange hover:underline">New to cooking?</button>
                 </div>
